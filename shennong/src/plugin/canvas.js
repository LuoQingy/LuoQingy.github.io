(function() {
	...
	//波浪的颜色
    var mWaveColor = "rgba(8,145,221,.4)";
	var mTextSize = 12;
	var mTextColor = "rgba(0,32,120,.8)";
	var ctx;
	var width = 0;
    var height = 0;
    var first = '00:00';
	...
	function drawCircle(obj) {
		...
		if(c) {
			var X = obj.width / 2;
			var Y = obj.height / 2;
		   	var radius = ((obj.width >= obj.height) ? (obj.height / 2) : obj.width / 2) - 2 * obj.mBorderwidth;
	...
	function drawText(obj) {
		...
		var info = obj.first;
        c.fillText(info, X - c.measureText(info).width / 2, Y-7, 100);
        c.fillRect( 6, Y+1,38,2);
        var Chapter = obj.second;
        c.fillText(Chapter, X - c.measureText(Chapter).width/2, Y+12, 100);
		c.fillStyle = obj.mWaveColor;
	};
